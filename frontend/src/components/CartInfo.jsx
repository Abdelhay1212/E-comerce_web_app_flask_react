...
const CartInfo = () => {
  ...
  const { cartItems, setCartItems, loading, setLoading, deleteCartItem } = useCart()
  ...
  useEffect(() => {
    ...
    if (cartItems && cartItems.length) {
      itemsTotal = cartItems.reduce((total, item) => total + Number(item.subtotal), 0)
    }
    setTotal(itemsTotal)
  }, [cartItems])
          ...
          {cartItems.length > 0 ? cartItems.map((item) => (
            <div key={item.id} className="flex flex-row justify-between items-center bg-white p-4 border-b border-l border-r">
              <div className="flex flex-row justify-center items-center w-3/6 text-center space-x-4">
                ...
                <Link to={`/product/${item.product.id}`} className="font-small text-[#607d8b] hover:text-gray-700 cursor-pointer pl-4">{item.product.title}</Link>
              </div>
